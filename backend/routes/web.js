...
router.post("/start", auth, async (req, res) => {
    try {
        ...
        // 1. Database Checks
        const findId = await query("SELECT * FROM web WHERE lab_id = ?", [labId]);
        ...
        const labNetworks = await query("SELECT * FROM web_networks WHERE lab_id = ?", [labId]);
        ...
        const containerSpecs = await query("SELECT container_name, role, network_name FROM web_containers WHERE lab_id = ?", [labId]);
