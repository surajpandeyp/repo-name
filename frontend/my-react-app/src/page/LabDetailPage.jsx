...
function LabDetailPage() {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (
          statusData.success &&
          statusData.running &&
          String(statusData.labId) === String(id)
        ) {
          setIsStarted(true);
  ...
  const handleDownloadVPN = async () => {
    ...
    try {
      const res = await fetch("/api/vpn/download", {
        method: "GET",
        headers: {
          Authorization: "Bearer " + token
        }
      ...
      if (!res.ok) {
        let message = "Failed to download VPN configuration.";
      ...
      // Backend se filename lena
      const contentDisposition = res.headers.get("Content-Disposition");

      let filename = "hackRangeVPN.ovpn";
      ...
      // Response ko Blob mein convert karo
      ...
      alert(
        err.message ||
        "VPN download failed. Please try again."
      );
    } finally {
      setVpnLoading(false);
    }
  ...
  const handleSubmitFlag = async () => {
    ...
    try {
      ...
      if (res.ok && data.success === true) {
        alert(data.message || "Flag verified successfully!");
        navigate("/feedback");
      } else {
        setFlagStatus(
          data.message || "Incorrect flag. Try again!"
        );
      }
      ...
      setFlagStatus(
        "Error connecting to server while verifying flag."
      );
    }
  };
    ...
    <div className="lab-page-wrapper">
      ...
      <div className="lab-top-nav">
        ...
        <div className="nav-left">
          ...
          {/* Dynamic VPN Download */}
          <button
            type="button"
            className={`nav-tab ${vpnLoading ? 'vpn-loading' : ''}`}
            onClick={handleDownloadVPN}
            disabled={vpnLoading}
            style={{
              border: "none",
              cursor: vpnLoading ? "wait" : "pointer"
            }}
          >
            {vpnLoading ? "⏳ Preparing VPN..." : "🔒 VPN"}
          </button>
          ...
          <>
            ...
            <div className="card briefing-banner-card">
              ...
              <div className="briefing-banner-info">
                ...
                <p>
                  Lab briefings contain critical
                  instructions and mission clues.
                </p>

              </div>

              <button
                className="btn-view-briefing"
                onClick={() =>
            ...
            <div className="card machine-control-card">
              ...
              <p className="machine-subtext">
                Start the machine to use Kali in-browser.
              </p>
            ...
            <div className="card task-card">
              ...
              <div className="flag-input-wrapper">
                ...
                <input
                  ...
                  value={userFlag}
                  onChange={(e) =>
                    setUserFlag(e.target.value)
                  }
                  placeholder={
                    isStarted
                      ? "Enter root flag..."
                      : "Start the machine to begin the lab..."
                  }
                  disabled={!isStarted}
                />

                <button
                  className="btn-flag-submit-icon"
