...
function Pivoting() {
  ...
  const startLab = async (lab) => {
    ...
    // LOADING ON
    setLoadingLabs((prev) => ({
      ...prev,   // ✅ FIXED (bprev -> prev)
      [lab.id]: true,
    }));

    try {
      const res = await fetch("http://localhost:3000/docker/start", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="pivot-page">
      <div className="pivot-header">
        <h1>Pivoting Labs</h1>
        <p>Practice internal network pivoting with real-world scenarios</p>
      </div>
        ...
        {pivotLabs.map((lab) => (
          <div className="pivot-card" key={lab.id}>
            ...
            <div className="pivot-right">
              <p className="ip-label">Lab IP Address</p>
